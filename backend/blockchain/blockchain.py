...
class Blockchain:
  ...
  def replace_chain(self, chain):
    """
    Replace the local chain with the incoming one if the follow applies:
  ...
  def from_json(chain_json):
    """
    Deserialze a list of serializes blocks into a Blockchain instance.
    ...
    for i in range(len(chain)):
      ...
      for transaction_json in block.data:
        ...
        if transaction.input == MINING_REWARD_INPUT:
          ...
          has_minig_reward = True
        else:
          historic_blockchan = Blockchain()
          historic_blockchan.chain = chain[0:i]
